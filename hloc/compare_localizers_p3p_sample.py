...
def main(dataset_name:str):
    ...
    sample_p3p_data = evaluate_sampled(model_path,results_paths['p3p'], [5, 25, 100, 200, 500, 1000, 5000, 7000, 10000], 30, list_file,ext=".txt")
    ...
    for i in range(plot_resolution):
        ...
        if num_non_zero != 0:
            t_err_time_vector[i] = sum / float(num_non_zero)
    ...
    for i in range(plot_resolution):
        ...
        if num_non_zero != 0:
            R_err_time_vector[i] = sum / float(num_non_zero)
    ...
    plt.title("Averaged+interpolated translation errors. Dataset : ShopFacade")
    plt.legend()

    plt.savefig("./plots/progression/translation_error.png", dpi=1300,
                bbox_inches='tight',
                facecolor='floralwhite')
    plt.close()
    plt.clf()
    ...
    plt.title("Averaged+interpolated rotation errors. Dataset : ShopFacade")
    plt.legend()

    plt.savefig("./plots/progression/rotation_error.png", dpi=1300,
                bbox_inches='tight',
                facecolor='floralwhite')
    plt.close()
    plt.clf()
